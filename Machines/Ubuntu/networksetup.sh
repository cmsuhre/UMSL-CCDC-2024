...
echo " Script will sleep for 10 seconds for allow changes to be completed"
sleep 10
echo " IF resolveconf is installed, any change manually done to /etc/resolv.conf will be lost as it gets overwritten next time something triggers resolvconf."
echo "EDIT /etc/resolv.conf if resolveconf is not running and installed, if it is, edit: [ sudo nano /etc/resolvconf/resolv.conf.d/base ]"
echo "DNS can be added in the file [ nano /etc/resolv.conf or nano /etc/resolvconf/resolv.conf.d/base ]"
echo "The FORMAT SHOULD LOOK LIKE: "
echo "-----------------------------------------------------------------"
dns="
nameserver	8.8.8.8 
